...
\begin{algorithmic}
	\State Modellgraph:$G_m$, Eingabegraph:$G_d$, Menge von Kandidaten: $K$
	\For {jede $p_i \in G_d$}
		\For {jeder Nachbar $p_j$ von $p_i$}
			\State Suchen alle Punktpaare $(v_k, v_l) \in G_m$, damit $dist(p_i, p_j) = dist(v_k, v_l)$ 
			\If {Anzahl der im letzten Schritt gefundenen Punktpaare 
				\\$>$ die Größe der $S_{min}$}
				...
				\If {Drei Nachbarn von $p_i$ können gefunden werden, 
					...
					\\und die identische Kante zum $p_i$ können in $G_m$ gefunden werden}
					\State Teilgraph Isomorphismus finden
				\EndIf
			\EndIf 
		\EndFor
	\EndFor
